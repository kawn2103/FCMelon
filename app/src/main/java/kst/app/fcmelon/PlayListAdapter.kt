...
class PlayListAdapter(private val callBack: (MusicModel) -> Unit) : ListAdapter<MusicModel,PlayListAdapter.ViewHolder>(diffUtil) {
    ...
    inner class ViewHolder(private val view: View):RecyclerView.ViewHolder(view ){
        fun bind(item: MusicModel){
            val trackTextView = view.findViewById<TextView>(R.id.itemTrackTv)
            val artistTextView = view.findViewById<TextView>(R.id.itemArtistTv)
            val coverImageView = view.findViewById<ImageView>(R.id.itemCoverIv)

            trackTextView.text = item.track
            artistTextView.text = item.artist
            Glide.with(coverImageView.context).load(item.coverUrl).into(coverImageView)
